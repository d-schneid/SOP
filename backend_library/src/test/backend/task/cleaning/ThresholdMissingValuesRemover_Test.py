...
class ThresholdMissingValuesRemoverTest(unittest.TestCase):
    ...
    def test_threshold_value_one(self):
        ...
        cleaned_dataset0: np.ndarray = self._ds.dataset0
        self.assertTrue(np.array_equal(cleaned_dataset0, self._one_threshold_remover.do_cleaning(self._ds.dataset0)), True)

        # Every entry has one None value -> remove every row
        self.assertEqual(self._one_threshold_remover.do_cleaning(self._ds.dataset1).shape[0], 0)
        ...
        self.assertEqual(self._one_threshold_remover.do_cleaning(self._ds.dataset3).shape[0], 0)

    def test_threshold_value_zero(self):
        """
        Tests ThresholdMissingValuesRemover with threshold 0 -> No rows are deleted
        """
        self.assertTrue(np.array_equal(self._ds.dataset0, 
                                       self._zero_threshold_remover.do_cleaning(self._ds.dataset0)), True)
        self.assertTrue(np.array_equal(self._ds.dataset1, 
        ...
        self.assertTrue(np.array_equal(self._ds.dataset3, 
                                       self._zero_threshold_remover.do_cleaning(self._ds.dataset3)), True)
    ...
    def test_threshold_half(self):
        ...
        cleaned_dataset0: np.ndarray = self._ds.dataset0
        self.assertTrue(np.array_equal(cleaned_dataset0, self._half_threshold_remover.do_cleaning(self._ds.dataset0)),
                        True)
        ...
        self.assertTrue(np.array_equal(cleaned_dataset1, self._half_threshold_remover.do_cleaning(self._ds.dataset1)),
                        True)
        ...
        # All rows consists of only None values -> Remove all rows
        self.assertEqual(self._half_threshold_remover.do_cleaning(self._ds.dataset3).shape[0], 0)
